...
func ChangePassword(userEmail, password string) *pb.ChangePasswordReply {
	...
	accountInfo, err := getLoginInfo(userInfoDB, userEmail)
	...
	checkHashPassword := helpers.GenerateHash(helpers.ConvertStringToByte(password),
	...
	if bytes.Compare(helpers.ConvertStringToByte(accountInfo.Password), checkHashPassword) == 1 {
		return &pb.ChangePasswordReply{Status: &pb.Status{Code: helpers.GetInvalidPasswordCode(), Message: "Current password is not valid"}}
	}
