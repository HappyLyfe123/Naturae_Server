...
func saveNewUser(wg *sync.WaitGroup, database *mongo.Database, collectionName string, user *user) {
	defer wg.Done()
...
func saveAuthenticationCode(wg *sync.WaitGroup, database *mongo.Database, collectionName string, newAuthenCode *userAuthentication) {
	...
	if err != nil {
		log.Println("Save authentication error: ", err)
	}
}
...
func sendAuthenticationCode(userEmail, firstName string, authenCode string) {
	//It going to try to keep authentication code if there an error
	for {
